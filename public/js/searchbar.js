...
//     }
//     $.ajax({
//         url: `/getDataFromIGDBcustomquery/${query}`,
//         type: 'GET',
//         //data: { q: query },
...
//                 external+=`
//                 <div class="game">
//                     <div style="background-image: url('{{ $game['coverurl'] }}'); height: 200px; background-size: cover; border-radius: 1.5rem;"></div>
//                     <div class="game-text">
//                         <h3>{{ $game['name'] }}</h3>
//                         <p>{{ \Illuminate\Support\Str::limit($game['summary'], 100) }}</p>
...
function loadExternalCollectionGames() {
    console.log('Loading external IGDB games for collection...');
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            $.each(res, function (index, ele) {
                ...
                external += `
                <div class="game">
                    <div style="background-image: url('${ele.coverurl}'); height: 200px; background-size: cover; border-radius: 1.5rem;"></div>
                    <div class="game-text">
                        <h3>${ele.name}</h3>
                        <p>${summary}</p>
                        <svg class="pull-left mar-sm-right" height="35px" space="preserve" style="" version="1.1" viewBox="0 0 419.3 482.7" width="40px" fill="#fff" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <button class="remove-from-library-btn" data-game-id="${ele.id}">
                            Remove from MyCollection
                        </button>
                    </div>
                </div>`;
            ...
            $('#gamesCollection').append(external);
            $('#loaderhere').empty();
        },
        error: function () {
            console.error('External library load for collection failed.');
            $('#loaderhere').empty();
        }
    });
}
...
$(document).ready(function() {
    ...
    // Load external IGDB games for the collection after the initial page render
    loadExternalCollectionGames();
});
            ...
            @else
                <ol>
                    @foreach($leaderboards[$category->id] as $index => $game)
                        <li style="margin: 0.5rem 0; font-weight: {{ $index === 0 ? 'bold' : 'normal' }}; color: {{ $index === 0 ? '#ffd700' : '#eee' }};">
                            <div style="display:flex; justify-content:center;">
                                {{ $game->title }} — {{ $game->votes }} votes
                            </div>
                            <div style="display:flex; justify-content:center;">
                                <img src="{{ $game->img }}" alt="{{ $game->title }}" style="margin-top: 0.5rem; width: 300px; max-width: 100%; border-radius: 10px; display: block;">
                            </div>
                        </li>
                    @endforeach
                </ol>
